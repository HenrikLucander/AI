...
def get_conditional_probability(data, query, condititions):
    ...
    # =========================================================
    #pass
    cond_prob = get_data_frequency(data,combined_dictionary)/get_data_frequency(data,condititions)+epsilon
    return (cond_prob)
    # =========================================================
    #return 0
...
def construct_probability_table(data, query, conditions):
    ...
    for assignment in assignments:
        ...
        # =========================================================
        #pass
        result.update({dict_key:get_conditional_probability(data,A,B)})
        # =========================================================
    return result
...
def exhaustive_enum(G, X, E, e):
    ...
    for assignment in assignments:
        ...
        for node in G.nodes:
            ...
            # =========================================================
            #break
            product = evaluate_node(G,node.data_id,combined_dictionary.get(node.data_id),combined_dictionary)*product
        total = product+total
            # =========================================================
    return total
...
def sample(G, X, E, e):
    ...
    for variable in X:
        ...
        if variable in E:
            ...
            #pass
            if value < 1:
                w = w * (1 - G.get_node_probability(variable, parents))
                assignment.append(value)
                variables.append(variable)
            else:
                w = w * G.get_node_probability(variable, parents)
                assignment.append(value)
                variables.append(variable)                
            # =========================================================
        else:
            # =========================================================
            #pass
            probability = G.get_node_probability(variable, parents)
